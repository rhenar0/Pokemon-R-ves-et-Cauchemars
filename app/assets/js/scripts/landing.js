 ...
 * 
 * @param {boolean} loading True if the loading area should be shown,
 * 
 *  otherwise false.
 */
...
function asyncSystemScan(mcVersion, launchAfter = true){
    ...
    sysAEx.on('message', (m) => {
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    'Aucune version<br>de Java trouvé',
                    'Pour rejoindre Seltade, vous avez besoin d\'une version de Java 8 en 64-bit. Voulez-vous que nous nous en occupions ?',
                    'Oui !',
                    'Je m\'en charge'
                )
                setOverlayHandler(() => {
                    setLaunchDetails('Préparation au Téléchargement...')
            ...
            if(m.result === true){
                ...
                setOverlayContent(
                    'Erreur Innatendu:<br>Téléchargemnt de Java impossible',
                    'Nous n\'avons pas réussi à installer Java... essayer plus tard ou procédez à son installation manuellement.',
                    'D\'accord'
                )
                setOverlayHandler(() => {
                    toggleOverlay(false)
    ...
    setLaunchDetails('Analyse des infos. systèmes...')
...
function dlAsync(login = true){
    ...
    if(login) {
        if(ConfigManager.getSelectedAccount() == null){
            loggerLanding.error('Vous devez être authentifié.')
            return
        }
    }
    ...
    aEx.on('error', (err) => {
        ...
        showLaunchFailure('Erreur lors du lancement', err.message || 'Vous pouvez utiliser la console (CTRL + Shift + i) pour avoir plus de détails.')
    })
    aEx.on('close', (code, signal) => {
        if(code !== 0){
            loggerLaunchSuite.error(`AssetExec exited with code ${code}, assuming error.`)
            showLaunchFailure('Erreur lors du lancement', 'Vous pouvez utiliser la console (CTRL + Shift + i) pour avoir plus de détails.')
        }
    })
        ...
        if(m.context === 'validate'){
            ...
            if(login && allGood) {
                ...
                try {
                    ...
                    setLaunchDetails('Amusez vous bien!')
